...
const UsersServices = {
    ...
    if (isadmin === true) {
      if (typeof queryLimit !== 'undefined') {
        const allAccounts = await dbConnection
          .dbConnect('SELECT * from users LIMIT $1',
            [queryLimit]);
        returnStatus = 200;
   ...
   * @param {*} queryLimit - Get query parameter
   */
  async getAllStaffs(staff, queryLimit) {
    let returnStatus; let returnSuccess = ''; let returnError = '';
    // check the users table
    const userDetails = await dbConnection
      .dbConnect('SELECT id, type, isadmin FROM users WHERE email=$1',
   ...
   * @param {*} queryLimit - Get query parameter
   */
  async getAllClients(staff, queryLimit) {
    let returnStatus; let returnSuccess = ''; let returnError = '';
    // check the users table
    const userDetails = await dbConnection
      .dbConnect('SELECT id, type, isadmin FROM users WHERE email=$1',
   ...
   * @constructor
   * @param {*} id - recieve account number.
   * @param {*} loggedIn - logged in details.
   */
  async getSpecificUser(id, loggedIn) {
    let returnStatus; let returnSuccess = ''; let returnError = '';
    ...
    const userAccount = await dbConnection
      .dbConnect('SELECT * from users WHERE id=$1',
        [id]);
